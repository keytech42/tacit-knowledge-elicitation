...
echo "[$(date -u -Iseconds)] Verifying backup: ${BACKUP_FILE}"
echo "[$(date -u -Iseconds)] Temporary database: ${TEMP_DB}"

cleanup() {
  echo "[$(date -u -Iseconds)] Cleaning up temporary database ${TEMP_DB}..."
...
echo "[$(date -u -Iseconds)] Expected tables (from live DB): ${EXPECTED_TABLES}"

# Create temporary database
echo "[$(date -u -Iseconds)] Creating temporary database..."
createdb -h "${PGHOST}" -U "${PGUSER}" "${TEMP_DB}"

# Restore into temp database
echo "[$(date -u -Iseconds)] Restoring backup into temporary database..."
...
echo "[$(date -u -Iseconds)] Restored tables: ${RESTORED_TABLES}"

# Compare
if [ "${RESTORED_TABLES}" -eq "${EXPECTED_TABLES}" ] && [ "${RESTORED_TABLES}" -gt 0 ]; then
  echo "[$(date -u -Iseconds)] PASS: Backup verification successful (${RESTORED_TABLES}/${EXPECTED_TABLES} tables)"
  exit 0
else
  echo "[$(date -u -Iseconds)] FAIL: Table count mismatch (restored: ${RESTORED_TABLES}, expected: ${EXPECTED_TABLES})" >&2
  exit 1
fi
